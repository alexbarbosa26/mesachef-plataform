...
Esta SPEC não pode mudar para `PRONTA_PARA_IMPLEMENTAR` enquanto as ADRs 0004, 0005 e 0006 estiverem em `PROPOSED` e as decisões críticas da seção 25 estiverem abertas. Esta revisão não autoriza código, instalação de dependências, migrations físicas, criação de credenciais, acesso a banco ou interface.
...
**Hipótese funcional — necessita validação.** Uma mesma pessoa precisa operar mais de uma empresa com uma única identidade e escolher a empresa ativa.

**Hipótese funcional — necessita validação.** `superadmin` administra a plataforma, mas não deve consultar automaticamente dados operacionais de clientes.
...
### 4.3 ADRs propostas e bloqueantes

- ADR 0004 — Kysely nos adapters de persistência, migrations versionadas e domínio independente;
- ADR 0005 — sessão opaca persistida no servidor, sem refresh token na aplicação web inicial;
- ADR 0006 — identidade global, membership multiempresa, RBAC e isolamento em profundidade.
...
- **Entrada:** ADR 0004 aceita e spike de query builder/migrator aprovado.
...
- [ ] ADR 0005 aceita ou substituída.
- [ ] ADR 0006 aceita ou substituída após spike de RLS.
- [ ] Associação multiempresa e seleção automática/obrigatória aprovadas.
- [ ] Matriz inicial de permissões e delegação aprovada.
- [ ] Limites de `superadmin`, MFA e bootstrap aprovados.
...
1. aceitar ADRs e concluir spikes em PostgreSQL 14 local;
...
## 25. Dúvidas abertas e decisões necessárias
...
| DEC-002-002 | Crítica | usuário em uma ou várias empresas | aceitar várias memberships e empresa ativa na sessão | 002-C |
| DEC-002-003 | Crítica | limite de `superadmin` | plataforma apenas, sem acesso operacional implícito | 002-D/E |
| DEC-002-004 | Crítica | matriz e delegação de permissões | aprovar códigos e matriz por ação; negar por padrão | 002-D |
| DEC-002-005 | Crítica | RLS com pool/query builder | executar spike com `SET LOCAL`, role real e teste de vazamento | 002-C/G |
...
A decomposição A–G, o modelo e os contratos estão suficientemente detalhados para decisão arquitetural, mas **não para implementação**. O próximo gate é uma revisão humana das ADRs 0004–0006 e das decisões `DEC-002-001` a `DEC-002-006`. Após aceite, uma execução separada e explicitamente autorizada poderá realizar somente os spikes técnicos necessários e atualizar esta SPEC; a spec ativa não deve avançar automaticamente.
